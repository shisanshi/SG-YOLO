...
def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    with builder.build_serialized_network(network, config) as engine, open(f, 'wb') as t:
        t.write(engine)  # 直接写入 engine
        print("Engine created:", engine)  # 这里应该打印一个 TensorRT 引擎，而不是 False
        if not engine:
...
# import tensorrt as trt
# import platform  # 这个在原代码中已经使用了
# def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False, int8=True, calib_data=None,
#                   calib_batch=1, prefix=colorstr('TensorRT:')):
#     # YOLOv5 TensorRT export https://developer.nvidia.com/tensorrt
#     assert im.device.type != 'cpu', 'export running on CPU but must be on GPU, i.e. `python export.py --device 0`'
#     try:
#         import tensorrt as trt
...
#             calibrator = Int8Calibrator(calib_data, calib_batch, cache_file=f'{file}_calibration.cache')
#             config.int8_calibrator = calibrator
#         else:
#             LOGGER.warning(
#                 f'{prefix} WARNING ⚠️ INT8 quantization requires calibration data, falling back to FP{16 if builder.platform_has_fast_fp16 and half else 32}')
#             int8 = False
#
#     # FP16 quantization
...
#         config.set_flag(trt.BuilderFlag.FP16)
#     elif not int8:
#         LOGGER.info(f'{prefix} building FP32 engine as {f}')
#
#     # Build engine
#     with builder.build_serialized_network(network, config) as engine, open(f, 'wb') as t:
#         t.write(engine)  # 直接写入 engine
#         LOGGER.info(f'{prefix} engine created: {f}')
#         if not engine:
#             raise RuntimeError("Failed to create TensorRT engine")
#     return f, None
...
#     def __init__(self, calib_data, batch_size, cache_file="calibration.cache"):
#         super().__init__()
#         self.calib_data = calib_data  # Calibration dataset
#         self.batch_size = batch_size
#         self.cache_file = cache_file
#         self.current_batch = 0
#         self.batches = len(self.calib_data) // self.batch_size
...
#         end_idx = min(start_idx + self.batch_size, len(self.calib_data))
#         batch = self.calib_data[start_idx:end_idx]
#
#         # Copy data to device
#         cuda.memcpy_htod(self.device_input, np.concatenate(batch))
...
def run(
        ...
        int8=True,  # CoreML/TF INT8 quantization
    ...
    if engine:  # TensorRT required before ONNX
        ...
        # f[1], _ = export_engine(model, im, file, half, dynamic, simplify, workspace, verbose,
        #                         int8=True, calib_data=calibration_dataset, calib_batch=8)
