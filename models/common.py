...
class Conv(nn.Module):
    ...
    default_act = nn.SELU()  # default activation
...
class MS_CAM(nn.Module):
    ...
    def __init__(self, channels, c2):
        ...
        self.global_att = nn.Sequential(
            ...
            nn.BatchNorm2d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inter_channels, c2, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(c2),
        )
...
class Bottleneck1(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.add = shortcut and c1 == c2
        self.ese = EffectiveSEModule(c1)
        self.ema = EMA(c_)
        self.mhsa = MHSA(c_)

    def forward(self, x):
        return x + self.cv2(self.mhsa(self.cv1(x))) if self.add else self.cv2(self.cv1(x))
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        # self.ese = EffectiveSEModule(c2)
        # self.ema = EMA(c2)

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))

class C3_M(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class SKFF(nn.Module):
    def __init__(self, in_channels=384, height=2, reduction=8, bias=False):
...
class FFM(nn.Module):
    ...
    def __init__(self,  dimension=1):
        super().__init__()
        self.d = dimension
        self.skff = SKFF(in_channels=384)

    def forward(self, x):
        out = self.skff(x)
        return out

class FFM_1(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self,  dimension=1):
        super().__init__()
        self.d = dimension
        self.skff = SKFF(in_channels=192)
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        #
        #     # 反序列化 engine
        #     if isinstance(engine, bytes):  # 确保 engine 是 bytes
        #         engine = runtime.deserialize_cuda_engine(engine)
        #
        #     # 确保 engine 是 ICudaEngine
        #     if not isinstance(engine, trt.ICudaEngine):
        #         raise TypeError("Failed to deserialize engine correctly")
        #
        ...
        #     batch_size = bindings['images'].shape[0]  # if dynamic, this is instead max batch size
            ...
            # 确保引擎反序列化成功
            if not isinstance(model, trt.ICudaEngine):
                raise TypeError("Failed to deserialize engine correctly")
            ...
            # 检查是否使用新版TensorRT API
            # 检查是否使用新版TensorRT API
            if hasattr(model, 'num_io_tensors'):  # 新版TensorRT API
                num_bindings = model.num_io_tensors
                for i in range(num_bindings):
                    tensor_name = model.get_tensor_name(i)
                    print(f"Tensor {i}: {tensor_name}")
                    name = tensor_name
                    # 修正这一行：使用tensor_name而不是索引i
                    dtype = trt.nptype(model.get_tensor_dtype(tensor_name))
                    print(f"DataType: {dtype}")
                    if model.get_tensor_mode(tensor_name) == trt.TensorIOMode.INPUT:
                        if -1 in tuple(model.get_tensor_shape(tensor_name)):  # dynamic
                            dynamic = True
                            context.set_input_shape(name, tuple(model.get_profile_shape(0, tensor_name)[2]))
                        if dtype == np.float16:
                            fp16 = True
                    else:  # output
                        output_names.append(name)
...
class ContextAggregation(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, reduction=1):
        ...
        self.channel_mul_conv = nn.Sequential(
            nn.Conv2d(self.inter_channels, in_channels//2, kernel_size=1),
...
class ContextBlock(nn.Module):
    def __init__(self, c1, c2, e=0.5):
        ...
        self.channel_mul_conv = nn.Sequential(
            nn.Conv2d(c2, c_, kernel_size=1),
            nn.LayerNorm([c_, 1, 1]),
            nn.ReLU(inplace=True),  # yapf: disable
            nn.Conv2d(c_, c2, kernel_size=1))
        # conv_params = dict(kernel_size=1, act_cfg=None)
        #
        # self.a = ConvModule(c1, 1, **conv_params)
        self.a = nn.Conv2d(c1, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        channel_add_term = self.channel_mul_conv(context)
